(() => {
  ...
  // index.js
  var IP_CHECK_API = "https://ipinfo.io/json";
  var US_COLOS_TO_TRY = ["IAD", "DFW", "LAX", "SJC", "EWR"];
  ...
  __name(getEgressIP, "getEgressIP");
  async function getBestEgressColo() {
    ...
    return { egressGeoData: bestGeoData, finalColo };
  }
  __name(getBestEgressColo, "getBestEgressColo");
  async function handleRequest(request) {
    ...
    <div style="background-color: #f0f8ff; border: 1px solid #dcdcdc; padding: 10px; margin-bottom: 15px; border-radius: 4px; font-size: 14px; text-align: left;">
        <h4 style="margin: 0 0 5px 0; color: #333;">Cloudflare Proxy Inf\xF3 \u{1F310}</h4>
        <ul style="list-style: none; padding: 0; margin: 0;">
            <li><strong>Bej\xF6v\u0151 (\xD6n \u27A1\uFE0F Worker) R\xE9gi\xF3:</strong> ${request.cf.colo} (${ingressGeoCountry})</li>
            <li><strong>Kimen\u0151 (Worker \u27A1\uFE0F C\xE9l) IP:</strong> <span style="color: ${egressGeoData.country === "US" ? "green" : "red"}; font-weight: bold;">${egressGeoData.ip}</span></li>
            <li><strong>Kimen\u0151 (C\xE9l) R\xE9gi\xF3:</strong> ${egressGeoData.city}, ${egressGeoData.country} (Kik\xE9nyszer\xEDtett Colo: ${finalColo})</li>
        </ul>
        <p style="margin: 5px 0 0 0; font-style: italic; color: #555;">(A k\xF3d ${US_COLOS_TO_TRY.length} USA Colot pr\xF3b\xE1lt ki a legjobb USA IP el\xE9r\xE9s\xE9hez. A ${finalColo} Colot haszn\xE1lja a f\u0151 k\xE9r\xE9shez.)</p>
    </div>
  `;
        ...
        if (url.pathname === "/proxy" && url.searchParams.has("url")) {
          ...
          if (path.startsWith("http://") || path.startsWith("https://")) {
            ...
            if (resolved) {
              ...
              let rebuilt = null;
              const ref2 = request.headers.get("Referer") || "";
              if (ref2.startsWith(`https://${url.host}/`)) {
                let refInner = ref2.substring(`https://${url.host}/`.length);
                if (refInner.startsWith(config.separator)) refInner = refInner.substring(config.separator.length);
                if (refInner.startsWith("http://") || refInner.startsWith("https://")) {
                  try {
    ...
    const isWikipediaSite = config.specialSites.wikipedia.enabled && config.specialSites.wikipedia.domains.some((domain) => targetURL.hostname.endsWith(domain));
    ...
    const isXHR = request.headers.get("X-Requested-With") === "XMLHttpRequest" || request.headers.get("Accept")?.includes("application/json");
    if (isXHR) {
    ...
    try {
      ...
      if (contentType.includes("text/html") || contentType.includes("application/xhtml+xml")) {
        const currentProxyDomain = url.host;
        let rewriter = new HTMLRewriter().on("a[href]", new LinkRewriter(targetURL, "href", currentProxyDomain)).on("form[action]", new LinkRewriter(targetURL, "action", currentProxyDomain)).on("img[src]", new LinkRewriter(targetURL, "src", currentProxyDomain)).on("img[srcset]", new SrcsetRewriter(targetURL, currentProxyDomain)).on("source[srcset]", new SrcsetRewriter(targetURL, currentProxyDomain)).on("link[href]", new LinkRewriter(targetURL, "href", currentProxyDomain)).on("script[src]", new LinkRewriter(targetURL, "src", currentProxyDomain)).on("iframe[src]", new LinkRewriter(targetURL, "src", currentProxyDomain)).on("source[src]", new LinkRewriter(targetURL, "src", currentProxyDomain)).on("video[src]", new LinkRewriter(targetURL, "src", currentProxyDomain)).on("audio[src]", new LinkRewriter(targetURL, "src", currentProxyDomain)).on("embed[src]", new LinkRewriter(targetURL, "src", currentProxyDomain)).on("object[data]", new LinkRewriter(targetURL, "data", currentProxyDomain)).on("track[src]", new LinkRewriter(targetURL, "src", currentProxyDomain)).on("meta[content]", new MetaContentRewriter(targetURL, currentProxyDomain));
  ...
  var BodyRewriter = class {
    static {
      __name(this, "BodyRewriter");
    }
  ...
  var SrcsetRewriter = class {
    static {
      __name(this, "SrcsetRewriter");
    }
  ...
  var MetaContentRewriter = class {
    static {
      __name(this, "MetaContentRewriter");
    }
    constructor(baseURL, proxyDomain) {
      this.baseURL = baseURL;
      this.proxyDomain = proxyDomain;
  ...
  var BaseTagRewriter = class {
    static {
      __name(this, "BaseTagRewriter");
    }
    constructor(baseURL, proxyDomain) {
      this.baseURL = baseURL;
      this.proxyDomain = proxyDomain;
  ...
  var StyleAttributeRewriter = class {
    static {
      __name(this, "StyleAttributeRewriter");
    }
    constructor(baseURL, proxyDomain) {
      this.baseURL = baseURL;
      this.proxyDomain = proxyDomain;
  ...
  var StyleElementRewriter = class {
    static {
      __name(this, "StyleElementRewriter");
    }
    constructor(baseURL, proxyDomain) {
      this.baseURL = baseURL;
      this.proxyDomain = proxyDomain;
  ...
  __name(rewriteJavaScript, "rewriteJavaScript");
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>CF Proxy Szolg\xE1ltat\xE1s</title>
  <style>
    body {
      font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
      max-width: 800px;
...
<body>
  <div class="container">
    <h1>CF Proxy Szolg\xE1ltat\xE1s</h1>
    
    ${ipInfoHtml} <p class="region-info">A k\xF3d megpr\xF3b\xE1lt garant\xE1ltan USA IP-t tal\xE1lni a list\xE1b\xF3l.</p>
  ...
  <script>
    function navigateToProxy(e) {
      ...
      // A separator \xFCres, \xEDgy a link tiszta lesz: /https://example.com
